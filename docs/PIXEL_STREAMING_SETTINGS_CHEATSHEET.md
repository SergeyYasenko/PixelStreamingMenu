...
## 🚀 LOW LATENCY MODE - Для отзывчивого управления

**Проблема:** Задержка управления камерой на планшете?  
**Решение:** Режим низкой задержки активирован! (см. `LOW_LATENCY_OPTIMIZATION.md`)

### Ключевые настройки для минимальной задержки:
```javascript
LowLatency: true              // Режим низкой задержки
RateControl: "CBR"            // Постоянный битрейт (не VBR!)
MaxLatency: 50                // Максимум 50мс (вместо 200мс)
TargetLatency: 30             // Цель 30мс
VideoScalingFactor: 0.75      // 75% разрешения для скорости
MinQP: 20, MaxQP: 37          // Баланс качества и скорости
FillerData: false             // Без лишних данных
```

**Результат:** Задержка ~50-100мс вместо 200-500мс! ✅
...
### Высокая задержка управления (камерой)
```javascript
LowLatency: true            // ✅ Режим низкой задержки
RateControl: "CBR"          // ✅ Постоянный битрейт
MaxLatency: 50              // ⬇️ Снизить до 50мс (было 200)
TargetLatency: 30           // ⬇️ Целевая 30мс
MinQP: 20                   // ✅ Минимум качества
MaxQP: 37                   // ✅ Максимум (было 51)
VideoScalingFactor: 0.75    // ⬇️ Уменьшить разрешение
FillerData: false           // ✅ Без заполнителей

// См. LOW_LATENCY_OPTIMIZATION.md для деталей
```
...
1. **Картинка "замораживается" на планшете?** → См. `TABLET_VIDEO_RENDERING_FIX.md` 🖼️
   - Проблема: GPU композитинг конфликтует
   - Решение: Убрали избыточные оптимизации, добавили `isolation: isolate`
2. **Задержка управления камерой?** → См. `LOW_LATENCY_OPTIMIZATION.md` ⚡
3. **Картинка замерла (не обновляется)?** → `KeyframeInterval: 500`
4. **Лагает?** → `WebRTCFPS: 24`, `VideoScalingFactor: 0.75`
5. **Артефакты?** → `MinBitrate: 1000000`, `MaxBitrate: 10000000`
6. **Не подключается?** → `LogLevel: "Verbose"`, проверь консоль
7. **H.264 не работает?** → `PreferVP8: true`, `ForceH264: false`

---

**Документация:**
- `TABLET_VIDEO_RENDERING_FIX.md` - 🖼️ Исправление "замораживания" видео на планшетах
- `LOW_LATENCY_OPTIMIZATION.md` - ⚡ Оптимизация задержки управления
- `TABLET_OPTIMIZATION_NOTES.md` - 📱 Общие оптимизации для планшетов
- `PIXEL_STREAMING_SETTINGS_CHEATSHEET.md` - 📋 Эта шпаргалка
